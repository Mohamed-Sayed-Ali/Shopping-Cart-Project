...
if page == 'Home':
    ...
    st.subheader('Dataset Overview') # (title) 
    st.dataframe(df)
    ...
    st.plotly_chart(px.line(data_frame=revenue_trend,x='order_date',y='total_price',color_discrete_sequence=['blue']))

    st.subheader('🏙️Top 10 State With Total Price')
    total_state=df.groupby('state')['total_price'].sum().reset_index().sort_values(by='total_price',ascending=False).head(10)
    st.plotly_chart(px.bar(data_frame=total_state,x='state',y='total_price',text_auto=True, labels= {'state' : 'State', 'total_price' : 'Total Revenue'},
       color_discrete_sequence=['red'],title='Top 10 States by Revenue'))
    st.write('---')  
    st.subheader("🔥 Top Products by Revenue")
    product_rev = df.groupby('product_name')['total_price'].sum().sort_values(ascending=False).head(10).reset_index()
    st.plotly_chart(px.bar(product_rev, x='product_name', y='total_price', title='Top 10 Products by Revenue',color_discrete_sequence=['yellow'],text_auto=True))
    ...
    product_count=df_filtered['product_name'].value_counts().reset_index().head(Top_N)
    st.plotly_chart(px.bar(data_frame=product_count,x='product_name',y='count',text_auto=True,color_discrete_sequence=['blue']))
